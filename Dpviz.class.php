...
class Dpviz extends \FreePBX_Helpers implements \BMO {
	...
	{
		$sql = sprintf("INSERT INTO %s (`panzoom`,`horizontal`,`datetime`,`destination`,`scale`,`dynmembers`) VALUES (1,0,1,1,1,0)", $this->table_name);
    ...
    public function editDpviz($panzoom, $horizontal, $datetime, $destination, $scale, $dynmembers) {
        $sql  = sprintf("UPDATE %s SET `panzoom` = :panzoom, `horizontal` = :horizontal, `datetime` = :datetime, `destination` = :destination, `scale` = :scale, `dynmembers` = :dynmembers WHERE `id` = 1", $this->table_name);
    ...
    public function doConfigPageInit($page) {
		...
		{
            case 'edit':
				$panzoom 	 = $request['panzoom'] ?? '';
				$horizontal  = $request['horizontal'] ?? '';
				$datetime 	 = $request['datetime'] ?? '';
				$destination = $request['destination'] ?? '';
				$scale 		 = $request['scale'] ?? '';
				$dynmembers  = $request['dynmembers'] ?? '';
	...
	public function showPage($page, $params = array())
	{
		...
		switch ($page) 
		{
			...
			case 'dialplan':
				// $data['iroutes'] = $this->dpp_load_incoming_routes();
				// $data['cid'] = $request['cid'] ?? '';
				// $data['iroute'] = sprintf("%s%s", $data['extdisplay'], $data['cid']);
				
				if (empty($data['extdisplay']))
				{
				...
				{
					$data['inroutes'] 			= $this->dpp_load_incoming_routes();
					$data['dproute']  			= $this->dpp_find_route($data['inroutes'], $data['iroute']);
					$data['clickedNodeTitle'] 	= $request['clickedNodeTitle'] ?? '';
	...
	public function getRightNav($request, $params = array())
	{
		...
		$data = array(
		 	...
		 	'destinationColumn' => ($options[0]['destination'] ?? '0') == 1 ? true : false,
